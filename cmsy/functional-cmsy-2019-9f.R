...
#-----------------------------------------
dataUncert   <-
  0.3  # set observation error as uncertainty in catch - default is SD=0.3
sigmaR       <-
  0.1 # overall process error for CMSY; SD=0.1 is the default
cor.log.rk   <-
  -0.607 # empirical value of log r-k correlation in 140 stocks analyzed with BSM (without r-k correlation)
rk.cor.beta  <- c(2.52, 3.37) # beta.prior for rk cor+1
nbk          <-
  3 # Number of B/k priors to be used by BSM, with options 1 (first year), 2 (first & intermediate), 3 (first, intermediate & final bk priors)
q.biomass.pr <-
  c(0.9, 1.1) # if btype=="biomass" this is the prior range for q
n            <-
  20000 #20000 # initial number of r-k pairs #reduced for testing
# n.new        <- n # initialize n.new
ni           <-
  3 # iterations for r-k-startbiomass combinations, to test different variability patterns; no improvement seen above 3
nab          <-
  3 # recommended=5; minimum number of years with abundance data to run BSM
bw           <-
  3 # default bandwidth to be used by ksmooth() for catch data
mgraphs      <-
  T # set to TRUE to produce additional graphs for management
e.creep.line <-
  T # set to TRUE to display uncorrected CPUE in biomass graph
kobe.plot    <-
  T # set to TRUE to produce additional kobe status plot; management graph needs to be TRUE for Kobe to work
BSMfits.plot <- T # set to TRUE to plot fit diagnostics for BSM
pp.plot      <-
  T # set to TRUE to plot Posterior and Prior distributions
retros       <-
  F # set to TRUE to enable retrospective analysis (1-3 years less in the time series)
save.plots   <- F # set to TRUE to save graphs to JPEG files
close.plots  <-
  F # set to TRUE to close on-screen plots after they are saved, to avoid "too many open devices" error in batch-processing
write.output <-
  T # set to TRUE if table with results in output file is wanted; expects years 2004-2014 to be available
write.pdf    <-
  F # set to TRUE if PDF output of results is wanted. See more instructions at end of code.
select.yr    <-
  NA # option to display F, B, F/Fmsy and B/Bmsy for a certain year; default NA
...
if (is.na(stocks[1]) == TRUE) {
  ...
  stocks         <-
    sort(as.character(cinfo$Stock)) # Analyze stocks in alphabetic order
...
for (stock in stocks) {
  cat("Processing",
      stock,
      ",",
      as.character(cinfo$ScientificName[cinfo$Stock == stock]),
      "\n")
...
set.seed(42)
  for (retrosp.step in 0:retros.nyears) {
    ...
    start.yr     <- min(cdat$yr[cdat$Stock == stock])
    ...
    user.log.r   <-
      ifelse(is.na(r.low) == F & is.na(r.hi) == F, TRUE, FALSE)
    ...
    if (mean(endbio) <= 0.5) {
      prior.k <-
        c(2 * max(ct) / mean(prior.r), 6 * max(ct) / mean(prior.r))
    } else {
    ...
    if (length(kv.all < 200 &&
               kv.all[kv.all < 1.1 * prior.k[1] & rv.all < mean(prior.r)]) > 20) {
    ...
    MSYs              <-
      rs[rs >= lcl.r.est &
           rs <= ucl.r.est & ks >= lcl.k.est & ks <= ucl.k.est] *
      ks[rs >= lcl.r.est &
           rs <= ucl.r.est & ks >= lcl.k.est & ks <= ucl.k.est] / 4
    MSY.est           <- median(MSYs)
    ...
    lcl.Fmsy.CMSY <-
      lcl.r.est / 2
    ucl.Fmsy.CMSY <- ucl.r.est / 2 # Fmsy from CMSY
    F_Fmsy.CMSY <-
      vector()
    lcl.F_Fmsy.CMSY <-
      vector()
    ucl.F_Fmsy.CMSY <- vector() # initialize vectors
  ...
  k <- exp(log.rk[2])
    ...
    if (btype == "CPUE" &
        FullSchaefer == T & e.creep.line == T & is.na(e.creep) == FALSE) {
      lines(
        x = yr,
    ...
    if (BSMfits.plot == T & FullSchaefer == T & force.cmsy == F) {
      ...
      points(
        ...
        resid,
        pch = 21,
        bg = ifelse(resid < lims[1] | resid > lims[2], 2, "white"),
        cex = 1
      )

      # save management chart to JPEG file
      if (save.plots == TRUE &
          FullSchaefer == T & BSMfits.plot == TRUE)
      {
    ...
    if (retrosp.step == 0) {
      ...
      output = data.frame(
        as.character(cinfo$Group[cinfo$Stock == stock]),
      ...
      output_names <- str_remove_all(colnames(output),"(as_character_cinfo_)|(ifelse_full_schaefer_t_)")
...
\\setlength\\parindent{0pt}
  ...
  \\end{document}"
      ...
      intro = (paste(intro, "\n\n", "Region: ", cinfo$Region[cinfo$Stock ==
                                                               stock], ".", sep = ""))
      intro = (paste(
        intro,
        "\n\n",
        "Marine Ecoregion: ",
        cinfo$Subregion[cinfo$Stock == stock],
        ".",
        sep = ""
      ))
      intro = (
        paste(
          intro,
          "\n\n",
          "Reconstructed catch data used from years ",
          min(yr),
          " - ",
          max(yr),
          sep = ""
      ...
      management_text <-
        paste(
          "\\\\textbf{Results for management (based on",
      ...
      management_text <-
        (
          paste(
            ...
            "\n\n",
            "Fmsy = ",
            format(Fmsy, digits = 3),
            ", 95% CL = ",
            format(lcl.Fmsy, digits = 3),
            " - ",
            format(ucl.Fmsy, digits = 3),
      ...
      management_text <-
        (
          paste(
            ...
            "\n\n",
            "Fmsy = ",
            format(Fmsy.last, digits = 3),
            ", 95% CL = ",
            format(lcl.Fmsy.last, digits = 3),
            " - ",
            format(ucl.Fmsy.last, digits = 3),
      ...
      management_text <-
        (
          paste(
            ...
            format(MSY, digits = 3),
            ",  95% CL = ",
            format(lcl.MSY, digits = 3),
            " - ",
            format(ucl.MSY, digits = 3),
            '; Bmsy = ',
            format(Bmsy, digits = 3),
            ",  95% CL = ",
            format(lcl.Bmsy, digits = 3),
            " - ",
            format(ucl.Bmsy, digits = 3),
            " (1000 tonnes)",
            sep = ""
          )
        )
      management_text <-
        (
          paste(
            ...
            format(B.last, digits = 3),
            ", 95% CL = ",
            format(lcl.B.last, digits = 3),
            " - ",
            format(ucl.B.last, digits = 3),
            " (1000 tonnes)",
            sep = ""
          )
        )
      management_text <-
        (
          paste(
            ...
            "\n\n",
            "B/Bmsy in last year = " ,
            format(B.Bmsy.last, digits = 3),
            ", 95% CL = ",
            format(lcl.B.Bmsy.last, digits = 3),
            " - ",
            format(ucl.B.Bmsy.last, digits = 3),
            sep = ""
          )
        )
      management_text <-
        (
          paste(
            ...
            format(F.last, digits = 3),
            ", 95% CL =",
            format(lcl.F.last, digits = 3),
            " - ",
            format(ucl.F.last, digits = 3),
            sep = ""
          )
        )
      management_text <-
        (
          paste(
            ...
            "\n\n",
            "F/Fmsy  = ",
            format(F.Fmsy.last, digits = 3),
            ", 95% CL = ",
            format(lcl.F.Fmsy.last, digits = 3),
            " - ",
            format(ucl.F.Fmsy.last, digits = 3),
            sep = ""
          )
        )
      management_text <-
        (paste(management_text, "\n\n", "Comment:", comment, ""))
      docTemplate <-
        gsub("#MANAGEMENT#", management_text, docTemplate)

      analysis_text <-
        (
          paste(
            "\\\\textbf{Results of CMSY analysis with altogether ",
            n.viable.b,
            " viable trajectories for ",
            n.viable.pt,
            " r-k pairs}\\\\\\\\",
      ...
      analysis_text <-
        (
          paste(
            ...
            "\n\n",
            "r = ",
            format(r.est, digits = 3),
            ", 95% CL = ",
            format(lcl.r.est, digits = 3),
            " - ",
            format(ucl.r.est, digits = 3),
            "; k = ",
            format(k.est, digits = 3),
            ", 95% CL = ",
            format(lcl.k.est, digits = 3),
            " - ",
            format(ucl.k.est, digits = 3),
            " (1000 tonnes)",
            sep = ""
          )
        )
      analysis_text <-
        (
          paste(
            ...
            "\n\n",
            "MSY = ",
            format(MSY.est, digits = 3),
            ", 95% CL = ",
            format(lcl.MSY.est, digits = 3),
            " - ",
            format(ucl.MSY.est, digits = 3),
            " (1000 tonnes/year)",
            sep = ""
          )
        )
      analysis_text <-
        (
          paste(
            ...
            format(median.btv.lastyr, digits = 3),
            " k, 95% CL = ",
            format(lcl.median.btv.lastyr, digits = 3),
            " - ",
            format(ucl.median.btv.lastyr, digits = 3),
            sep = ""
          )
        )
      analysis_text <-
        (
          paste(
            ...
            format((F.CMSY / Fmsy.CMSY)[length(median.btv) - 1], digits = 3),
            sep = ""
          )
        )
        ...
        analysis_text <-
          (
            paste(
              analysis_text,
              "\n\n",
              "\\\\textbf{Results from Bayesian Schaefer model using catch and ",
              btype,
              "}\\\\\\\\",
              sep = ""
            )
        ...
        analysis_text <-
          (
            paste(
              ...
              "\n\n",
              "r = ",
              format(r.jags, digits = 3),
              ", 95% CL = ",
              format(lcl.r.jags, digits = 3),
              " - ",
              format(ucl.r.jags, digits = 3),
              "; k = ",
              format(k.jags, digits = 3),
              ", 95% CL = ",
              format(lcl.k.jags, digits = 3),
              " - ",
              format(ucl.k.jags, digits = 3),
              sep = ""
            )
          )
        analysis_text <-
          (
            paste(
              ...
              "\n\n",
              "MSY = ",
              format(MSY.jags, digits = 3),
              ", 95% CL = ",
              format(lcl.MSY.jags, digits = 3),
              " - ",
              format(ucl.MSY.jags, digits = 3),
              " (1000 tonnes/year)",
              sep = ""
            )
          )
        analysis_text <-
          (
            paste(
              ...
              format(quant.P[2, ][nyr], digits = 3),
              " k, 95% CL = ",
              format(quant.P[1, ][nyr], digits = 3),
              " - ",
              format(quant.P[3, ][nyr], digits = 3),
              sep = ""
            )
          )
        analysis_text <-
      ...
      analysis_text <-
        (
          paste(
            ...
            "\n\n",
            "q = ",
            format(mean.q, digits = 3),
            ", 95% CL = ",
            format(lcl.q, digits = 3),
            " - ",
            format(ucl.q, digits = 3),
            sep = ""
          )
        )
        ...
        analysis_text <-
          (
            paste(
              ...
              "\n\n",
              "Prior range of q = ",
              format(q.prior[1], digits = 3),
              " - ",
              format(q.prior[2], digits = 3),
              sep = ""
            )
          )
      }
        ...
        analysis_text <-
          (
            paste(
              ...
              "\n\n",
              "Biomass = ",
              format(B.sel, digits = 3),
              ", B/Bmsy = ",
              format(B.Bmsy.sel, digits = 3),
              ", fishing mortality F = ",
              format(F.sel, digits = 3),
              ", F/Fmsy = ",
              format(F.Fmsy.sel, digits = 3),
              sep = ""
            )
          )
      }
      ...
      analysis_text <-
        (paste(
          analysis_text,
          "\n\n",
          "Relative abundance data type = ",
          format(btype, digits = 3),
          sep = ""
        ))
      analysis_text <-
        (
      ...
      analysis_text <-
        (
          paste(
            analysis_text,
            "\n\n",
            "Prior intermediate relative biomass = ",
            intbio[1],
            " - ",
            intbio[2],
            " in year ",
      ...
      analysis_text <-
        (
          paste(
            ...
            "\n\n",
            "Prior range for r = ",
            format(prior.r[1], digits = 2),
            " - ",
            format(prior.r[2], digits = 2),
            ifelse(is.na(r.low) == T, " default", " expert"),
            ", prior range for k = " ,
            format(prior.k[1], digits = 3),
            " - ",
            format(prior.k[2], digits = 3),
            " (1000 tonnes) default",
            sep = ""
          )
        )
      ...
      docTemplate <-
        gsub("#IMAGE1#", managementchartfile, docTemplate)
      ...
      documentfile <-
        paste(stock,
              ...
              "-",
              sub(":", "", substr(as.character(Sys.time(
              )), 12, 16)),
              ".RnW",
              sep = "") # concatenated hours and minutes added to file name
    ...
    FFmsy.retrospective[[retrosp.step + 1]] <-
      F.Fmsy #retrospective analysis
    BBmsy.retrospective[[retrosp.step + 1]] <-
      B.Bmsy #retrospective analysis
    years.retrospective[[retrosp.step + 1]] <-
      yr #retrospective analysis
