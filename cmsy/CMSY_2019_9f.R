...
#	}
  ...
  if(grepl("windows",tolower(Sys.info()['sysname']))) {windows(14,9)}
  ...
  if(mgraphs==T) {
    ...
    if(grepl("windows",tolower(Sys.info()['sysname']))) {windows(14,12)}
    par(mfrow=c(2,2))
  ...
  if(pp.plot==T) {
    # open window for plot of four panels
    if(grepl("windows",tolower(Sys.info()['sysname']))) {windows(17,12)}
    # make margins narrower
  ...
  if(BSMfits.plot==T & FullSchaefer==T & force.cmsy==F){
    #---------------------------------------------
    # open window for plot of four panels
    if(grepl("windows",tolower(Sys.info()['sysname']))) {windows(9,6)}
    # make margins narrower
  ...
  if(kobe.plot==T){
    # open window for plot of four panels
    if(grepl("windows",tolower(Sys.info()['sysname']))) {windows(7,7)}
    par(mfrow=c(1,1))
    # make margins narrower
...
\\setlength\\parindent{0pt}
  ...
  years.retrospective[[retrosp.step+1]]<-yr #retrospective analysis
	...
	if (retros == T){
	   ...
	   if(grepl("windows",tolower(Sys.info()['sysname']))) {windows(14,7)}
